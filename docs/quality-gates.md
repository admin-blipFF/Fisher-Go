...
- A second water-outline diagnostic was rejected after a matched API 35
  repeat on `emulator-5554` using the Intel UHD host renderer. Removing only
  the Game 3D water fill outline measured p95 `53 ms` / `46.49%` jank versus
  `48 ms` / `46.85%` for the restored control; Flutter total p95 also rose to
  `89 ms` versus `62 ms`. Slow draw commands and bitmap uploads were lower,
  but the frame-time result was worse and both variants missed the `20 ms`
  gate. The diagnostic flag and style helper were removed, leaving the full
  OSM water/road/pier/building layer family as the production default. Evidence:
...
- On 2026-08-01, the notification opt-in boundary was added without coupling
  ...
  gate.
- On 2026-08-03, `notification_opt_in_flow_test.dart` passed twice on API 35
  `emulator-5554`, including the clean-install runner path. With no catches the
  Profile screen showed no notification prompt; after a seeded first catch it
  showed `釣點及活動提醒` and `開啟通知`, and `稍後` removed the prompt. The
  preflight explicitly skipped API 23 serial `0123456789ABCDEF`.
...
| Current Android native map baseline | Valid evidence; motion gate open | The current profile APK on API 35 `emulator-5554` rendered the native MapLibre Surface with the host GPU and produced 411 valid frames: p95 `62 ms`, p99 `110 ms`, `18.49%` jank, no ANR. This isolates a native renderer cost that remains even before Flutter GameHome composition; API 23 was excluded. Evidence: `tmp/android-native-map-benchmark-profile-asset-cleanup-api35.json`. |
| Current Android GameHome profile baseline | Valid evidence; motion gate open | A current-source profile APK built with `FISHERGO_MAP_PERF=true` reached the GPS-centered GameHome on API 35 host GPU and produced 269 valid frames: gfx p95 `61 ms`, p99 `81 ms`, `47.21%` jank, Flutter total p95 `93 ms`, no ANR. Evidence: `tmp/android-map-benchmark-current-profile-perf-continuation-20260803.json`; API 23 remained excluded. |
| Latest API 35 current-source recheck | Valid evidence; motion gate open | On 2026-08-04, the existing current-source profile APK was rerun on API 35 `emulator-5554` with host-GPU and map-readiness gates. It produced `236` valid native frames, gfx p95/p99 `61/77 ms`, `49.58%` jank, `105` slow UI-thread frames, `14` slow bitmap uploads, `69` slow draw commands, Flutter total p95 `105 ms`, and no ANR. API 23 serial `0123456789ABCDEF` was not targeted. Evidence: `tmp/android-map-benchmark-current-continuation-20260804.json`. |
...
| API 35 smoke | Pass | On `emulator-5554`, fresh API 35 runs passed startup/auth, map rotation, GPS-centered spot selection, bite-first minigame, Daily Task reward, and notification opt-in sequencing; API 23 remained excluded by preflight. |
...
| Fresh API 35 current-source rebaseline | Valid trace; gate failed | On 2026-08-03, API 35 `emulator-5554` with Intel UHD 630 host GPU produced `642` native frames after map readiness: gfx p95/p99 `32/48 ms`, `28.35%` janky, `66` slow UI-thread frames, `9` slow bitmap uploads, `94` slow draw commands, Flutter total p95 `43 ms`, raster p95 `24 ms`, and no ANR. The direct native SurfaceView isolation also measured p95/p99 `38/250 ms`, confirming that the native map renderer remains above the 20 ms target independently of Flutter HUD composition. API 23 serial `0123456789ABCDEF` was excluded. Evidence: `tmp/android-map-benchmark-masterplan-current-20260803.json` and `tmp/android-native-map-benchmark-masterplan-current-20260803.json`. |
| Fresh current-source rebaseline after notification boundary | Valid trace; gate failed | On 2026-08-03, the readiness-gated GameHome profile trace on API 35 `emulator-5554`/Intel UHD 630 recorded `336` native frames, gfx p95/p99 `48/69 ms`, `44.05%` janky, Flutter total p95 `118 ms`, style/map idle `2761/2768 ms`, and no ANR. Enabling native player/spot layers did not improve it: p95 stayed `48 ms` and jank rose to `46.74%`; both candidates were rejected. Evidence: `tmp/android-map-benchmark-current-source-api35-20260803.json` and `tmp/android-map-benchmark-native-marker-layers-api35-20260803.json`. |
...
| Opaque building extrusion candidate | Reject - no stable improvement | Keeping the same real building geometry and heights but changing extrusion opacity from `0.66` to opaque left gfx p95 at `32 ms`, while worsening slow UI frames `66→81`, slow bitmap uploads `9→10`, slow draw commands `94→103`, and Flutter total p95 `43→45 ms`; no ANR occurred. The opt-in flag and style were removed. Evidence: `tmp/android-map-benchmark-opaque-building-20260803.json`. |
...
| Android fill-antialias candidate | Reject - no stable GameHome win | The candidate disabled `fill-antialias` only on the Android Game 3D `fill` layers while preserving OSM water, landcover, roads, piers, and building extrusion. Native proof repeats improved from `62 ms` / `18.97%` jank to `58 ms` / `14.32%`, but the complete GameHome A/B moved from `40 ms` / `43.99%` jank / Flutter p95 `53 ms` to `46 ms` / `43.74%` / `61 ms`, with a repeat at `42 ms` / `41.39%` / `56 ms` and more draw commands (`105` to `112/116`). The style was removed from production; the 20 ms gate remains open. Evidence: `tmp/android-native-map-benchmark-control-repeat-20260803.json`, `tmp/android-native-map-benchmark-no-fill-aa-repeat-20260803.json`, `tmp/android-map-benchmark-gamehome-fill-aa-control-20260803.json`, `tmp/android-map-benchmark-gamehome-fill-aa-disabled-20260803.json`, and `tmp/android-map-benchmark-gamehome-fill-aa-disabled-repeat-20260803.json`. |
| Android native player/spot overlay candidate | Reject - no stable composition win | Existing native MapLibre player and spot layers were measured with Flutter markers disabled while keeping the original fill-antialias style. The GameHome trace stayed at gfx p95 `40 ms` versus the `40 ms` control, jank was `44.01%` versus `43.99%`, Flutter p95 total was `52 ms` versus `53 ms`, and slow draw commands increased from `105` to `128`; no ANR occurred. The flags remain opt-in diagnostics only because the candidate does not close the motion gate or improve the full overlay path. Evidence: `tmp/android-map-benchmark-gamehome-native-marker-layers-20260803.json` and `tmp/android-map-benchmark-gamehome-fill-aa-control-20260803.json`. |
...
| Android line-motion geometry recheck | Reject - repeatable regression | A current-source API 35 Intel UHD 630 host-GPU pair kept the same OSM layers and camera while changing line caps/joins from round to butt/miter. Control/candidate p95 was `42/46 ms` on the first pair and `42/46 ms` on the repeat; candidate jank was `45.51%/46.07%` versus control `46.55%/46.57%`, while draw commands moved `120/85` and `95/94` and there was no ANR. The candidate reduced some draw-command work but made total motion slower, so it was removed and the production style remains unchanged. Evidence: `tmp/android-map-benchmark-flat-line-control-20260803.json`, `tmp/android-map-benchmark-flat-line-candidate-20260803.json`, `tmp/android-map-benchmark-flat-line-candidate-repeat-20260803.json`, and `tmp/android-map-benchmark-flat-line-control-repeat-20260803.json`. |
...
It also captures the full GPS accessibility `content-desc` when Android
combines the spoken location label with the compact `GPS 15m` label, keeping
the explicit recenter action tappable for API 35 motion traces.
...
| Latest deployed production smoke | Pass release/visual boundary; wide-screen motion gate still fails | After GitHub auto-deployed commit `69f3963` to Vercel Production, both `fisher-go.app` and `www.fisher-go.app` returned HTTP 200, both `/assets/.env` paths returned 404, and both release manifests reported `0.1.1+2-69f396306579`. The live Web runner received OpenFreeMap tiles and passed populated-map goldens at 390x844 (`56.84 FPS`, p95 `33.3 ms`, `1.41%` jank) and 1440x900 (`42.23 FPS`, p95 `50.1 ms`, `19.34%` jank). The wide-screen frame budget remains open. Evidence: `tmp/web-map-benchmark-production-masterplan-20260803.json` and `dart run tool/verify_deployed_web.dart --url=https://fisher-go.app --alias=https://www.fisher-go.app`. |
| Web road-layer merge candidate | Reject - no stable desktop motion gain | A valid query-gated candidate merged `road-medium` and `road-major` into one `road-main` layer while preserving OpenFreeMap tiles and both visual goldens. Matched desktop control/candidate p95 stayed `66.7/66.7 ms`; FPS was `36.03/35.60` on the first pair and `36.06/35.08` on the repeat. Mobile and desktop jank did not improve consistently, so the query and source contract were removed. The first malformed nested zoom-expression attempt was also rejected by MapLibre and corrected before the valid A/B. Evidence: `tmp/web-map-benchmark-road-merge-control-390-1440.json`, `tmp/web-map-benchmark-road-merge-candidate-390-1440.json`, `tmp/web-map-benchmark-road-merge-control-repeat-390-1440.json`, and `tmp/web-map-benchmark-road-merge-candidate-repeat-390-1440.json`. |
| Web raster-basemap candidate | Reject - no stable desktop motion gain and weak game visual | A valid query-gated Carto `light_nolabels` raster style preserved GPS camera, rotation, marker projection, real OSM-derived roads/water/land, successful raster tile responses, and both structural visual goldens. The first matched desktop pair measured control `41.31 FPS` / p95 `66.6 ms` / `19.42%` jank versus candidate `39.53 FPS` / p95 `50.0 ms` / `23.86%`; the repeat measured control `41.54` / `50.1 ms` / `20.77%` versus candidate `36.00` / `66.6 ms` / `26.67%`. Mobile stayed near `56-57 FPS` without a stable gate closure. The candidate was removed because the desktop result did not repeat, jank worsened, and the very pale raster style did not meet the FisherGO game-map visual direction. Evidence: `tmp/web-map-benchmark-raster-control-390-1440.json`, `tmp/web-map-benchmark-raster-candidate-390-1440-v4.json`, `tmp/web-map-benchmark-raster-control-repeat-390-1440.json`, and `tmp/web-map-benchmark-raster-candidate-repeat-390-1440.json`. |
| Web native player sprite candidate | Reject - no stable motion gain and weaker marker treatment | A build-gated Web candidate registered the selected avatar body asset in a MapLibre-native `MarkerLayer` and preserved OpenFreeMap tile responses, populated visual goldens, GPS centering, and compass rotation. The first matched pair measured mobile `54.99/55.38 FPS` with p95 `33.3/33.3 ms` and desktop `39.78/39.89 FPS` with p95 `66.6/66.6 ms`; repeats measured mobile `54.30/54.64 FPS` and desktop `38.40/40.36 FPS`, with desktop jank `20.42%/21.39%`. Browser inspection confirmed the candidate showed the selected body but lost the existing marker's glow/navigation treatment, so the Web flag and asset path were removed. Evidence: `tmp/web-map-benchmark-native-player-control-390-1440.json`, `tmp/web-map-benchmark-native-player-candidate-390-1440.json`, `tmp/web-map-benchmark-native-player-control-repeat-390-1440.json`, `tmp/web-map-benchmark-native-player-candidate-repeat-390-1440.json`, and `output/playwright/web-map-proof-390x844.png`. |
| Web motion-only pixel-ratio candidate | Reject - no admissible trace and lifecycle instability | A new query-gated `?fishergo_motion_pixel_ratio=0.5` candidate attempted to lower WebGL resolution only during camera motion and restore the baseline at rest. The first implementation triggered a MapLibre near/far matrix error and an `idle`/`setPixelRatio` recursion; the load-gated correction removed the recursion, but the formal runner still timed out waiting for `flutter-view` and produced no valid candidate frame trace. The candidate was removed; the current adaptive `0.75` idle preset remains unchanged. Evidence: `tmp/web-map-benchmark-motion-pixel-control-390-1440-20260803.json` and `tmp/web-map-benchmark-motion-pixel-candidate-diagnostic-20260803.json`. |
| Web canvas antialias candidate | Reject - no stable gate improvement | A query-gated `?fishergo_canvas_antialias=0` candidate disabled MapLibre WebGL canvas MSAA while keeping the same style, camera, GPS, tile source, and marker contract. Both matched pairs passed populated visual goldens and HTTP 200 OpenFreeMap checks. Desktop repeats remained at p95 `66.6/66.7 ms` with `21.54/21.00%` and `22.70/21.05%` jank; mobile first-run FPS fell from `56.24` to `51.19` before the repeat returned near control. The hook was removed because canvas antialiasing is not the primary release-gate bottleneck. Evidence: `tmp/web-map-benchmark-canvas-antialias-control-20260803.json`, `tmp/web-map-benchmark-canvas-antialias-candidate-20260803.json`, `tmp/web-map-benchmark-canvas-antialias-control-repeat-20260803.json`, `tmp/web-map-benchmark-canvas-antialias-candidate-repeat-20260803.json`, and `tmp/web-map-benchmark-canvas-antialias-diagnostic-20260803.json`. |
| Web responsive-zoom candidate | Reject - wide-screen jank remains above budget | A same-build camera experiment kept mobile at zoom `16.0`, tested wide-screen zoom `17.0` and `17.75`, and preserved the OpenFreeMap source plus both visual goldens. The `17.0` repeats reached about `36 FPS` / p95 `66.6 ms` / `26.11–28.57%` jank at 1440x900; `17.75` reached `31.57 FPS` / p95 `50.1 ms` / `40.13%` jank. Although the tighter scale reduced p95 in one run, it worsened jank and did not meet the 30 FPS, 33.3 ms, and 5% targets together. The helper and tests were removed; default zoom remains `16.0`. Evidence: `tmp/web-map-benchmark-wide-zoom-20260803.json`, `tmp/web-map-benchmark-wide-zoom-20260803-repeat.json`, and `tmp/web-map-benchmark-wide-zoom-1775-20260803.json`. |
...
| Web current tile-cache depth 2 candidate | Reject - no current-source improvement | The query-gated `?fishergo_tile_cache_levels=2` preserved OpenFreeMap responses, bootstrap budget, and the 1440x900 visual golden, but the current-source control/candidate measured `40.57/39.90 FPS`, p95 `66.6/66.6 ms`, and jank `18.32/19.60%`; candidate long-task total also increased `1748/1879 ms`. Retain MapLibre's default dynamic cache policy. Evidence: `tmp/web-map-benchmark-current-control-continuation-20260803.json` and `tmp/web-map-benchmark-current-tile-cache-2-candidate-continuation-20260803.json`. |
...
| Web release protected-input preflight | Pass as source guard; hosted dispatch remains owner-controlled | `.github/workflows/web-release.yml` now checks the Vercel token, public Supabase URL/anon key/project ref, and privacy/support metadata immediately after checkout and before Flutter/Node setup. The source contract passes; no hosted production dispatch was performed because the protected environment is owner-controlled. |
...
| Android merged landcover fill diagnostic | Matched API 35 host-GPU control/candidate both gfx p95 `32 ms`; control/candidate slow draw commands `88/110`; janky `29.39%/28.16%`; no ANR | Reject. Merging grass, wood, and farmland into one filtered fill did not improve the p95 motion gate and increased draw commands. The opt-in flag, style transformer, and tests were removed; production geography and the default renderer are unchanged. Evidence: `tmp/android-map-benchmark-landcover-control-20260803.json` and `tmp/android-map-benchmark-landcover-candidate-20260803.json`. |
| Current-source textureless direct Hybrid Composition | Valid API 35 host-GPU GameHome proof with HUD and map visible; gfx p95 `53 ms`, `96.63%` janky, 89 frames, no ANR | Reject. `FISHERGO_MAP_TEXTURE=false` with direct HC did not remove the composition tail; it worsened p95 and Flutter total p95 to `105 ms`. Keep texture mode and the retained `tlhc_vd` path as the product default. Evidence: `tmp/android-map-benchmark-hc-textureless-current-20260803.json` and `tmp/android-hc-textureless-current-20260803.png`. |
| Native building visibility toggle during camera motion | Control/candidate native SurfaceView p95 `42/44 ms`, janky `9.66%/10.94%`, p99 `300/78 ms`, no ANR | Reject. Hiding only `building-3d` on camera-start and restoring it on camera-idle reduced the extreme p99 tail but worsened the release-relevant p95 and jank. The profile-only callback and benchmark switch were removed; stationary 3D buildings and full geography remain unchanged. Evidence: `tmp/android-native-map-benchmark-building-motion-control-20260803.json` and `tmp/android-native-map-benchmark-building-motion-toggle-20260803.json`. |
| Android building extrusion base diagnostic | First matched gfx p95 `34/38 ms`, repeat `42/40 ms`, janky `35.50%/33.77%` and `37.12%/36.29%`, no ANR | Reject. Replacing the real `render_min_height` base lookup with `0` did not produce a stable release-relevant p95 win. Keep the dynamic base and full building geometry. Evidence: `tmp/android-map-benchmark-building-base-control-api35.json`, `tmp/android-map-benchmark-building-base-candidate-api35.json`, `tmp/android-map-benchmark-building-base-control-api35-repeat.json`, and `tmp/android-map-benchmark-building-base-candidate-api35-repeat.json`. |
| Android Flutter markers hidden during camera motion diagnostic | First candidate gfx p95 `61 ms` / `48.43%` janky versus current profile baseline `61 ms` / `47.21%`; warm repeat `53 ms` / `48.94%` janky; no ANR | Reject. Pausing Flutter marker projection reduced Flutter telemetry p95 in both runs, but did not produce a stable native frame-time or jank improvement. The opt-in flag and code path were removed; markers remain visible and interactive after every idle event. Evidence: `tmp/android-map-benchmark-candidate-hide-markers-during-motion-20260803.json` and `tmp/android-map-benchmark-candidate-hide-markers-during-motion-repeat-20260803.json`. |
...
The read-only audit was rerun locally on 2026-08-04 without building,
deploying, or mutating Supabase: `4` checks passed, `0` failed, `2` remained
